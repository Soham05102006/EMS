...
import { Thermometer, TreePalmIcon, UmbrellaIcon, Plus } from 'lucide-react'
    ...
    <div className='Animate-fade-in'>
         ...
         sm:items-center gap-4 mb-8">
                  <div>
                    ...
                    <p className="page-subtitle">{isAdmin ? "Manage Leave Application.":"Your Leaves history and requests."}</p>
                  </div>
                  {!isAdmin && !isDeleted && (
           ...
           <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8">
                     {leaveStats.map((card) => (
                       //const Icon = card.icon
                        ...
                        <div key={card.label} className="card card-hover p-5 sm:p-6 relative overflow-hidden 
                        ...
                        <div className='p-3 bg-slate-100 rounded-lg group-hover:bg-indigo-50
                         transition-colors duration-200'> 
                            <card.icon className='w-5 h-5 text-slate-600 group-hover:text-indigo-600
                            transition-colors duration-200'/>
                        </div>
                            <div>
                                   <p className='text-sm text-slate-500'>{card.label}</p>
                                   <p className='text-2xl font-medium tracking-tight text-slate-900'>   {card.value}<span className='text-sm font-normal text-slate-400'> taken</span></p>
