...
export const inngest = new Inngest({ id: "fullstacks-ems" });
...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!attendance?.checkOut){
        ...
        //after 10 hours, mark the attendance as checkout with status "LATE"
        await step.sleepUntil("Wait-for 1-hour", new Date(new Date().getTime() + 1 * 60 * 60 * 1000));
    
           attendance = await Attendance.findById(attendanceId)    
           if(!attendance?.checkOut){
            attendance.checkOut = new Date(attendance.checkIn).getDate() + 4 * 60 * 60 * 1000;
...
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron", triggers: [{ cron: "TZ=Asia/Kolkata 30 11 * * *" } ] }, // 11:30 AM IST (06:00 UTC)
  async ({step }) => {
    // Implementation for checking attendance and emailing absent employees
